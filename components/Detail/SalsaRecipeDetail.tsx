...
// ✅ IMPORTACIÓN CLAVE: Se necesita useRoute para acceder a los parámetros
import { useRoute, RouteProp } from '@react-navigation/native'; 

// Se estandariza la interfaz de la Receta
...
type RootStackParamList = {
    SalsaRecipeDetail: { recipe: Recipe };
};

// ✅ CORRECCIÓN 1: Se elimina { recipe } de las props ya que se usará useRoute
export default function SalsaRecipeDetail() {
  
  // ✅ CORRECCIÓN 2: Se usa useRoute para obtener el objeto recipe
  const route = useRoute<RouteProp<RootStackParamList, 'SalsaRecipeDetail'>>();
  ...
  // ✅ ROBUSTEZ: Se usa 'recipe ? ... : false' porque 'recipe' podría ser nulo aquí
  ...
  const handleFavoritePress = async () => {
    // ✅ ROBUSTEZ: Se comprueba que la receta exista
    if (!recipe) return;
  ...
  // Si la receta es null (porque aún no ha cargado o no se pasó), mostramos el cargador.
...
// Estilos estandarizados y combinados (sin cambios)
